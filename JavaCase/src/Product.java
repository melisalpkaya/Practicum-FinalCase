...
public class Product {
    ...
    public Product(int id, double unitPrice, double discountRate, int stock, String model, String nameBrand, int storage, double screenSize, int RAM) {
        ...
        this.brand = new Brands(nameBrand);
        this.storage = storage;
        this.screenSize = screenSize;
        this.RAM = RAM;
    }

    public Product(int id, double unitPrice, double discountRate, int stock, String model, String nameBrand, int storage, double screenSize,int battery, int RAM, String color) {
        this.id = id;
        this.unitPrice = unitPrice;
        this.discountRate = discountRate;
        this.stock = stock;
        ...
        if (num%2 != 0) {
            ...
            if (!productList.isEmpty()) {
                for (Product product : productList) {
                    ...
                    System.out.printf("| %-2s | %-30s| %-10s TL | %-10s| %-12s | %-10s | %-10s | %-10s | %-10s |\n" ,
                            product.getId(), product.getProductName(), product.getUnitPrice(), product.getNameBrand(),
                            product.getStorage(), product.getScreenSize(),  product.getBattery(), product.getRAM(), product.getColor());
                }
                System.out.println("-------------------------------------------------------------------------------------------------------------------------------------------------");
    ...
    public static void addItems(int num) {
        ...
        if (num%2 != 0) {
            ...
            Product newMobilePhone = new Product(id, unitPriceInput, discountRateInput, stockInput, modelInput, brandInput, storageInput, screenSizeInput, batteryInput, RAMInput, colorInput);
    ...
    private static void listFilteredItems(int num, ArrayList<Product> filteredProductList) {
        if (num%2 != 0) {
            ...
            if (!filteredProductList.isEmpty()) {
                for (Product product : filteredProductList) {
                    System.out.printf("| %-2s | %-30s| %-15s TL | %-10s| %-10s| %-12s | %-10s | %-10s | %-10s |\n" ,
                            product.getId(), product.getProductName(), product.getUnitPrice(), product.getNameBrand(),
                            product.getStorage(), product.getScreenSize(), product.getBattery(), product.getRAM(), product.getColor());
                }
                System.out.println("------------------------------------------------------------------------------------------------------------------------------------------------------");
